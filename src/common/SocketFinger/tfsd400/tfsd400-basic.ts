...
import {SocketUtils} from "../../../public/socket-utils";
import * as _ from "lodash";
...
export class TfsD400Basic extends SocketUtils {
    public isDebugMode: boolean;
    public deviceId: string
    private sendMode:string //发送模式 就是发了什么命令就是什么模式
    ...
    constructor(fingerConf: FingerConf) {
        ...
        this.myevents.on("onData", (message: string) => {
            ...
            if (buf[0] == 0xF5) {
                //F5
                switch (buf[1]) {
                    case 0x01: //01 第1次录入指纹
                        this.myevents.emit("record1", buf);
                        break;
                    case 0x02: //02 第2次录入指纹
                        this.myevents.emit("record2", buf);
                        break;
                    case 0x03: //03 第3次录入指纹
                    ...
                    case 0x0C: //0C 比对1：N
                        ...
                        break;
                    case 0x26: //26 取DSP 模块版本号
                        this.myevents.emit("getVersion", buf);
                        break;
                    case 0x28: //28 取DSP 比较等级
                        this.myevents.emit("compareGrade", buf);
                        break;
                    case 0x24: //24 采集图像并上传
                        this.myevents.emit("getImageAndUpload", buf);
                        break;
                    case 0x23: //23 采集图像并提取特征值上传
                        this.myevents.emit("getImageFeatureUpload", buf);
                        break;
                    case 0x2B: //2B 取已登录所有用户用户号及权限
                        this.head = buf.subarray(0,8);
                        console.log("head:",this.head.toString('hex'));
                        const result = Q3[this.head[4]]; //buf[4] 0:ACK_SUCCESS,1:ACK_FAIL
                        if(buf.length==8) throw Error(`getRegAllUserInfo error:${Q3[this.head[4]]}`)
                        this.body=buf.subarray(8);
                        this.total = this.concatHighLow(this.body[1], this.body[2]);
                        console.log(127,this.head.toString('hex'),this.body.toString('hex'),this.total);
                        if(buf.length == 13 + this.total*3) {
                            this.myevents.emit("getRegAllUserInfo", buf);
                        } else {
                            this.tempBuf = buf;
                        }
                        break;
                    case 0x2C: //2C 使模块进入休眠状态
                        this.myevents.emit("sleep", buf);
                        break;
                    case 0x2D: //2D 设置/读取指纹添加模式
                        this.myevents.emit("fingerAddMode", buf);
                        break;
                    case 0x2E: //2E 设置/读取指纹采集等待超时时间
                        this.myevents.emit("overTime", buf);
                        break;
                    case 0x44: //44 下传特征值与采集指纹比对
                    ...
                    case 0x43: //43 下传指纹特征值与DSP 模块数据库指纹比对1：N
                        ...
                        break;
                    case 0x31: //31 上传DSP 模块数据库内指定用户特征值
                        this.myevents.emit("uploadDspOne", buf);
                        break;
                    case 0x41: //41 下传特征值并按指定用户号存入DSP 模块数据库
                ...
                console.log(`可能是某些数据长的命令的分包数据: total:${this.total},tempBuf:${this.tempBuf.length},buf:${buf.length},${this.tempBuf[0]},${this.tempBuf[1]}`);
                if (this.tempBuf[0] == 0xF5 && this.tempBuf[1] == 0x2B) {
                    buf = Buffer.concat([this.tempBuf,buf]);
                    // console.log(`buf len:${buf.length}`);
                    if (buf.length == (13 + this.total * 3)) {
                        this.myevents.emit("getRegAllUserInfo", buf);
                        this.tempBuf = Buffer.from([]);
                    } else {
                        this.tempBuf = buf;
                    }
                } else {

                    console.log(this.tempBuf.toString('hex'));
                }
            }
        });
    }
    ...
    private concatHighLow(high: number, low: number): number {
        return parseInt(
            high.toString(16).padStart(2, "0") + low.toString(16).padStart(2, "0"), 16);
    }
    ...
    async getTotalCapacity(): Promise<ResTotal> {
        this.sendMode='getTotalCapacity'
        return new Promise((resolve) => {
            //发送报文
            ...
            this.myevents.once("getTotalCapacity", (buf) => {
                ...
                const result = buf[4]==0xFF ? 'ACK_SUCCESS':'ACK_FAIL' ;
    ...
    async compareOneToMore(): Promise<ResTotal> {
        this.sendMode='compareOneToMore'
        return new Promise((resolve) => {
    ...
    getImageAndUpload(): Promise<ResTotal> {
        this.sendMode='getImageAndUpload'
        return new Promise((resolve) => {
            //计算命令
    ...
    getImageFeatureUpload(): Promise<ResTotal> {
        this.sendMode='getImageFeatureUpload'
        return new Promise((resolve) => {
            //计算命令
    ...
    downloadFeatureAndCompareToGetFeature(fno: number): Promise<ResTotal> {
        this.sendMode='downloadFeatureAndCompareToGetFeature'
        return new Promise((resolve) => {
            //计算命令
            let userBytes = this.getUserBytes(fno);
            //发送报文
            ...
            if (this.isDebugMode)
                console.log("downloadFeatureAndCompareToGetFeature", cmd);
            this._send(cmd); //发送报文
            //接收报文后的响应
            // <Buffer f5 09 0f a0 ff 00 59 f5>
    ...
    downloadFeatureAndCompareOneToOne(fno: number): Promise<ResTotal> {
        this.sendMode='downloadFeatureAndCompareOneToOne'
        return new Promise((resolve) => {
            //计算命令
            let userBytes = this.getUserBytes(fno);
            //发送报文
            ...
            if (this.isDebugMode)
                console.log("downloadFeatureAndCompareOneToOne", cmd);
            this._send(cmd); //发送报文
            //接收报文后的响应
            // <Buffer f5 09 0f a0 ff 00 59 f5>
    ...
    downloadFeatureAndCompareOneToMore(feature: string): Promise<ResUser> {
        this.sendMode='downloadFeatureAndCompareOneToMore'
        return new Promise((resolve) => {
            //计算命令
            ...
            let cmd = head + body;
            if (this.isDebugMode)
                console.log("downloadFeatureAndCompareOneToMore", cmd);
            this._send(cmd); //发送报文
            //接收报文后的响应
            // <Buffer f5 09 0f a0 ff 00 59 f5>
        ...
        return new Promise((resolve, reject) => {
            ...
            this.myevents.once("uploadDspOne", (buf: Buffer) => {
                ...
                if (buf.length == 8) {
                    ...
                    console.assert(
                        feature.length === 193 * 2,
                        `feature长度错误长度应为386，但现在为${feature.length}`,
                    );
    ...
    downloadFeatureAndSaveToDsp(fno: number, feature: string): Promise<ResTotal> {
        this.sendMode='downloadFeatureAndSaveToDsp'
        return new Promise((resolve) => {
            ///计算命令 head
    ...
    getRegAllUserInfo(): Promise<ResData<UserInfo>> {
        this.sendMode='getRegAllUserInfo'
        return new Promise((resolve, reject) => {
            //发送报文
